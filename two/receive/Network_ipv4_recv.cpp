...
const char *accept_ip[2] = {"255.255.255.255", "192.168.0.1"};
u_int16_t ip_id = 0;
u_int16_t i = 0;
...
int network_ipv4_recv(u_int8_t *ip_buffer)
{
	...
	u_int16_t fragment;
	fragment = ntohs(ip_hdr->fragment_offset); // ntohs 用于将网络字节序转换为主机字节序

	int dural = 0;
	if (previous == 0) // 第一次接收到数据包
	{
		previous = time(NULL);
	}
	else
	...
	else if (ip_id == ip_hdr->id)
	{
		load_data_to_buffer(buffer + i, ip_buffer + sizeof(ip_header), len);
		i += len;
		FILE *fp = fopen("data_receive.txt", "w");
